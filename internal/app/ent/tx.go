...
func (tx *Tx) init() {
	...
	tx.FirstLogin = NewFirstLoginClient(tx.config)
	tx.LoginAttempts = NewLoginAttemptsClient(tx.config)
	tx.Password = NewPasswordClient(tx.config)
	tx.Permission = NewPermissionClient(tx.config)
	tx.Phone = NewPhoneClient(tx.config)
	tx.Role = NewRoleClient(tx.config)
	tx.RoleHasPermissions = NewRoleHasPermissionsClient(tx.config)
	tx.Services = NewServicesClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.UserHasRoles = NewUserHasRolesClient(tx.config)
}
...
// applies a query, for example: Ban.QueryXXX(), the query will be executed
