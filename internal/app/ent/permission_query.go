...
func (pq *PermissionQuery) QueryRoleHasPermissions() *RoleHasPermissionsQuery {
	query := (&RoleHasPermissionsClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withRoleHasPermissions: pq.withRoleHasPermissions.Clone(),
...
func (pq *PermissionQuery) WithRoleHasPermissions(opts ...func(*RoleHasPermissionsQuery)) *PermissionQuery {
	query := (&RoleHasPermissionsClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withRoleHasPermissions = query
	return pq
}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withRoleHasPermissions; query != nil {
		if err := pq.loadRoleHasPermissions(ctx, query, nodes,
			func(n *Permission) { n.Edges.RoleHasPermissions = []*RoleHasPermissions{} },
			func(n *Permission, e *RoleHasPermissions) {
...
func (pq *PermissionQuery) loadRoleHasPermissions(ctx context.Context, query *RoleHasPermissionsQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *RoleHasPermissions)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(rolehaspermissions.FieldPermissionID)
	}
	query.Where(predicate.RoleHasPermissions(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(permission.RoleHasPermissionsColumn), fks...))
	}))
