...
type RoleQuery struct {
	...
	withPermission         *UserQuery
	withRoleHasPermissions *RoleHasPermissionsQuery
...
func (rq *RoleQuery) QueryPermission() *UserQuery {
...
// QueryRoleHasPermissions chains the current query on the "role_has_permissions" edge.
func (rq *RoleQuery) QueryRoleHasPermissions() *RoleHasPermissionsQuery {
	query := (&RoleHasPermissionsClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) QueryUserHasRoles() *UserHasRolesQuery {
	query := (&UserHasRolesClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withRoleHasPermissions: rq.withRoleHasPermissions.Clone(),
		withUserHasRoles:       rq.withUserHasRoles.Clone(),
...
func (rq *RoleQuery) WithPermission(opts ...func(*UserQuery)) *RoleQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithRoleHasPermissions tells the query-builder to eager-load the nodes that are connected to
// the "role_has_permissions" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithRoleHasPermissions(opts ...func(*RoleHasPermissionsQuery)) *RoleQuery {
	query := (&RoleHasPermissionsClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoleHasPermissions = query
	return rq
}
...
func (rq *RoleQuery) WithUserHasRoles(opts ...func(*UserHasRolesQuery)) *RoleQuery {
	query := (&UserHasRolesClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withUserHasRoles = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			rq.withRoleHasPermissions != nil,
			rq.withUserHasRoles != nil,
		}
	)
	...
	if query := rq.withPermission; query != nil {
		if err := rq.loadPermission(ctx, query, nodes,
			func(n *Role) { n.Edges.Permission = []*User{} },
			func(n *Role, e *User) { n.Edges.Permission = append(n.Edges.Permission, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withRoleHasPermissions; query != nil {
		if err := rq.loadRoleHasPermissions(ctx, query, nodes,
			func(n *Role) { n.Edges.RoleHasPermissions = []*RoleHasPermissions{} },
			func(n *Role, e *RoleHasPermissions) {
				n.Edges.RoleHasPermissions = append(n.Edges.RoleHasPermissions, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := rq.withUserHasRoles; query != nil {
		if err := rq.loadUserHasRoles(ctx, query, nodes,
			func(n *Role) { n.Edges.UserHasRoles = []*UserHasRoles{} },
			func(n *Role, e *UserHasRoles) { n.Edges.UserHasRoles = append(n.Edges.UserHasRoles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadPermission(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Role)
	nids := make(map[int]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(role.PermissionPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.PermissionPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.PermissionPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (rq *RoleQuery) loadRoleHasPermissions(ctx context.Context, query *RoleHasPermissionsQuery, nodes []*Role, init func(*Role), assign func(*Role, *RoleHasPermissions)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(rolehaspermissions.FieldRoleID)
	}
	query.Where(predicate.RoleHasPermissions(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.RoleHasPermissionsColumn), fks...))
	}))
...
func (rq *RoleQuery) loadUserHasRoles(ctx context.Context, query *UserHasRolesQuery, nodes []*Role, init func(*Role), assign func(*Role, *UserHasRoles)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userhasroles.FieldRoleID)
	}
	query.Where(predicate.UserHasRoles(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.UserHasRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
